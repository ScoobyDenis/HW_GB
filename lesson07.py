...
from random import *
matrix_1 = [[randint(-10, 10) for _ in range(5)] for _ in range(4)]
matrix_2 = [[randint(-10, 10) for _ in range(5)] for _ in range(4)]
...
class Matrix:
    ...
    def __str__(self):
        print('first matrix')
        [print(*i) for i in self.matr1]
        print("second matrix")
        [print(*i) for i in self.matr2]

    def __add__(self):
        print("The sum of matrixs")
        for i in range(len(self.matr1)):
            for j in range(len(self.matr1[i])):
                self.matr1[i][j] = self.matr1[i][j] + self.matr2[i][j]
        for i in range(4):
            for j in range(4):
                print(str(self.matr1[i][j]).ljust(5), end=" ")
            print()

m = Matrix(matrix_1, matrix_2)
m.__str__()
m.__add__()

# 2. Реализовать проект расчёта суммарного расхода ткани на производство одежды.
# Основная сущность (класс) этого проекта — одежда, которая может иметь
# определённое название. К типам одежды
# в этом проекте относятся пальто и костюм. У этих типов одежды существуют
# параметры: размер (для пальто) и рост (для костюма). Это могут быть обычные
...
class Coat(Clothes):
    ...
    def size(self, size):
        if size < 30:
            self.__size = 'Извините,мы не шьем пальто размером меньше 30'
        elif size > 65:
            self.__size = 'Извините,мы не шьем пальто размером больше 65'
        else:
            self.__size = f'Нам понадобится {round((size/6.5 + 0.5), 2)} метров ткани'
...
class Costume(Clothes):
    def __init__(self, height):
        self.height = height
        #height = ceil(2*self.height + 0.3)
        #print(f'Расход ткани на костюм {v} сантиметр(а)')
        print(height)
    @property
    def height(self):
    ...
    def height(self, height):
        if height > 200:
            self.__height = 'Мы не шьем костюмы на рост выше 200см'
        elif height < 140:
            self.__height = 'Мы не шьем костюмы на рост ниже 140см, пройдите в детские отдел'
        else:
            self.__height = f'Нам понадобится {2*height + 0.3}см ткани'

    def info(self):
        return self.__height

a = Coat(float(input('size: ')))
print(a.info())
b = Costume(float(input('height: ')))
print(b.info())
...
class Cell:
    ...
    def __str__(self):
        return f'Количество клеток после операции {self.num * "*"}'

    def make_order(self, column):
        for i in range(0, self.num - self.num % column, column):
            print('*' * column)
        print('*' * (self.num % column))
